...
const telegramBotTokken = '6076259682:AAEwFW9MwcwDGID0yXIeiKoR6ctNeDzx16k'
const telegramApiHost = 'api.telegram.org'
const telegramApiPath = '/bot'+telegramBotTokken
const webhookHost = 'example.com'
...
function requestHandler(req, res){
  if(req.url === '/'){
    ...
    console.log('requested to: '+req.url+' -> not found')
    res.writeHead(404)
...
function setWebhook(){
  const reqOptions = {
    ...
    path: telegramApiPath+'/setwebhook?url='+webhookHost+webhookPath
  }
  https.request(reqOptions, (res)=>{
    let rawData = ''
    res.on('data', (chunk) => {rawData += chunk})
    res.on('end', () => {console.log('setWebhook() response:\n\t'+rawData); getWebhookInfo()})
